...
# 找到所有包含fa-folder的li（分类）
for li in soup.find_all('li'):
    folder_icon = li.find('i', class_='fa-folder')
    if folder_icon:
        # 这是一个分类
        cat_a = li.find('a', recursive=False)
        if cat_a:
            cat_name = cat_a.get_text(strip=True)
            # 找到这个分类下的ul
            sub_ul = li.find('ul', class_='menu', recursive=False)
            if sub_ul:
                # 找到所有书签链接
                for sub_li in sub_ul.find_all('li', recursive=False):
                    ie_icon = sub_li.find('i', class_='fa-internet-explorer')
                    if ie_icon:
